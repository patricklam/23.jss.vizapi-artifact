...
Java has two different mechanisms for gathering dynamic information, Java agents and the Java Virtual Machine Tool Interface (JVMTI).

\paragraph{Java agents}
Java agents along with Java instrumentation APIs (\texttt{java.lang.instrument interface}) allow you to modify bytecode of classes.
Different libraries such as ASM and Javassist provide APIs for bytecode manipulation, we choose Javassist for VizAPI.  
...
Since the agent also runs in the JVM, Java agents are portable.
We use a Java agent for VizAPI's dynamic analysis because it allows for straightforward bytecode manipulation 
and also because our dynamic analysis focuses on API interactions and not memory management.
...
This means that this is a good choice for monitoring JVM internals such as memory and garbage collection without introducing overhead from the agent itself.
However, portability is affected in this case since it is a native interface.
