...
free graph layout software, we chose to use d3graph\footnote{https://pypi.org/project/d3graph/} which is a Python library for D3~\cite{bostock2011} to present our graphs due to the number of visualization techniques the toolkit supports, the fact that visualizations are deployable to the web, the ease of modifications of the Python code, and the very good uptake within the community. There are also other variants of D3 for different purposes such as Vega~\cite{satyanarayan2016}, VegaLite~\cite{satyanarayan2017}, and Plotly\footnote{https://plotly.com/}, among others, with bindings for different languages.

Our visualizations are still mostly-static representation of a single graph. However,
d3graph renders to HTML, allows the viewer to interact by rotating the view, moving, and selecting nodes, and supports multiple different types of graph techniques. We found it useful to be able to change the viewing angle of more complicated graphs. d3graph makes it easy to add more interaction capabilities and to capture evolution aspects of the data in the future. 
